...
import "./interface/IAirdropERC20Claimable.sol";
    ...
    IAirdropERC20Claimable
{
     ...
     *                                        allowlist can claim.
     */
    function claim(address _receiver, uint256 _quantity, bytes32[] calldata _proofs, uint256 _proofMaxQuantityForWallet)
        external
        nonReentrant
    {
    ...
    function verifyClaim(
        ...
        if (merkleRoot != bytes32(0)) {
            (isOverride,) = MerkleProof.verify(
                _proofs, merkleRoot, keccak256(abi.encodePacked(_claimer, _proofMaxQuantityForWallet))
            );
        }
